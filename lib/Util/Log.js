...
class Logger extends EventEmitter {
  ...
  error(msg) {
    if (typeof msg == 'object' && msg.stack) {
      msg.stack = msg.stack.replace(botPathRegEx, '.');
      if (this._token) msg.stack = msg.stack.replace(new RegExp(this._token, 'g'), 'TOKEN_WAS_HERE');
    }
    this.log('error', msg);
  }
  ...
  emit(log) {
    ...
    this._socket.emit('log', log.replace(/(\[)\w+(m)/g, ''));
  }
}
