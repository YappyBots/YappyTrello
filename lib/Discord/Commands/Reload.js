...
class ReloadCommand extends Command {
  ...
  run(msg, args) {
    ...
    if (!command && bot.aliases.has(argName)) {
      ...
      return msg.channel.send(`No command name was provided to reload`);
    } else if (!command) {
      msg.channel.send(`Command \`${argName}\` doesn't exist, checking file...`);
    }
    ...
    fs.stat(filePath, (err, stats) => {
      if (err) return msg.channel.send(`I cannot find the file ${fileName}`);
      if (!stats.isFile()) return msg.channel.send(`Command \`${cmdName}\` is not a file`);
      msg.channel.send(`Reloading ${fileText} \`${cmdName}\`...`).then(m => {
        if (!command) {
        ...
        Log.error(e);
        msg.channel.send(`Unable To Reload Command \`${cmdName}\``);
      });
    });
  }
